const APIController = (function() {
    ...
    const artistaId = 'Taylor';
    const  _getArtistsSearch = async (token) => {

        console.log("Prueba Artista");
        const result = await fetch(`https://api.spotify.com/v1/search?q=${artistaId}&type=artist`, {
            method: 'GET',
            headers: { 'Authorization' : 'Bearer ' + token}
        });
    ...
    //FUNCION OBTENER A LOS ALBUM
    const _getAlbumSearch = async (token) => {

        console.log("Prueba Album");
    ...
    //FUNCION OBTENER A LOS TRACKS
    const _getTracksSearch = async (token) => {

        console.log("Prueba Track");
    ...
    //FUNCION OBTENER A LOS PODCAST
    const _getEposidoSearch = async (token) => {
    ...
    return {
        ...
        getEposidoSearch(token){
            return _getEposidoSearch(token);
        }
    }
})();
...
const APPController = (function(UICtrl, APICtrl) {
    ...
    DOMInputs.submitArtist.addEventListener('click', async (e) => {
        ...
        const token = UICtrl.getStoredToken().token;        
        // get artists
        await APICtrl.getArtistsSearch(token);
